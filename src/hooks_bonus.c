...
/*   Updated: 2022/03/08 17:00:11 by syolando         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	destroy_game(t_game *game, char *s)
{
	...
	mlx_destroy_image(game->mlx, game->img.black.img);
	mlx_destroy_image(game->mlx, game->img.boss.img);
...
{
	destroy_game(game, "Game is closed with mouse");
	return (0);
}
...
int	is_step_possible(t_game *game, int dx, int dy)
{
	...
	if (game->map[game->y_p + dy][game->x_p + dx] == 'B')
		destroy_game(game, "You Lose!");
	return (1);
}
...
{
	if (key == ESC)
		destroy_game(game, "Game is closed whith ESC!");
